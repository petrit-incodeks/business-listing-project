...
class AitItemCpt extends AitPublicCpt
{
	...
	public function __construct($id, $config, $paths)
	{
		...
		add_action("delete_{$this->itemCategoryId}", array($this, 'removeExtraItemCategoryFormFields'), 10, 4);
		...
		add_action("delete_{$this->itemLocationId}", array($this, 'removeExtraItemLocationFormFields'), 10, 4);
	...
	public function saveExtraItemCategoryFormFields($term_id)
	{
		if(isset( $_POST[$this->itemCategoryId])){
			...
			// checkbox not sent through $_POST when not checked, so manual check
			if(!isset($_POST[$this->itemCategoryId]['category_featured'])){
				$extraFields['category_featured'] = '';
			}
	...
	public function removeExtraItemCategoryFormFields($term_id)
	{
		delete_option("{$this->itemCategoryId}_category_{$term_id}");
	}
	...
	public function saveExtraItemLocationFormFields($term_id)
	{
		if ( isset( $_POST[$this->itemLocationId] ) ) {
			...
			// checkbox not sent through $_POST when not checked, so manual check
			if(!isset($_POST[$this->itemLocationId]['category_featured'])){
				$extraFields['category_featured'] = '';
			}
	...
	public function removeExtraItemLocationFormFields($term_id)
	{
		delete_option("{$this->itemLocationId}_category_{$term_id}");
	}
	...
	public function addItemTaxonomiesSpecialPages($specialPages)
	{
		...
		foreach ($this->getRawPublicTaxonomies() as $taxonomy) {
			$pages["_taxonomy_{$taxonomy->name}"] = array(
				...
				'with-id' => false,
				'if' => function() use($taxonomy) { return is_archive() && is_tax($taxonomy->name); },
			);
		}
	...
	public static function saveItemMeta($postId, $post, $metabox, $data, $update)
	{
		if($post->post_type == 'ait-item'){
			...
			if(empty($data)){
				$data = get_post_meta($postId, '_ait-item_item-data', true);
			}

			if(!empty($data['map'])){	// fixed notice
