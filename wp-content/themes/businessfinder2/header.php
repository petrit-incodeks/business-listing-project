				...
				<div class="header-container grid-main">
					...
					<div class="site-logo">
						{if $options->theme->header->logo}
							{var $logo_alt = "logo"}
							{if attachment_url_to_postid($options->theme->header->logo) !==0}
								{var $logo_alt = get_post_meta( attachment_url_to_postid($options->theme->header->logo), '_wp_attachment_image_alt', true)}
								{if $logo_alt == ""}
									{var $logo_alt = "logo"}
								{/if}
							{/if}

						<a href="{$homeUrl}" title="{!$wp->name}" rel="home"><img src="{$options->theme->header->logo}" alt="{$logo_alt}"></a>
						{else}
