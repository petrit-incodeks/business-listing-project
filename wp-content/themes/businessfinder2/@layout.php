...
{if $wp->isTax(items) or $wp->isTax(locations) or $wp->isSingular(item) or $wp->isSearch}
	{if $headerLayoutType == 'map' and $elements->unsortable[header-map]->display and $elements->unsortable[search-form]->display}
		<div class="ait-toggle-area-group">
			{if $headerLayoutType == 'map' and $elements->unsortable[header-map]->display}
			...
			{if $elements->unsortable[search-form]->display}
				<a href="#" class="ait-toggle-area-btn" data-toggle=".elm-search-form"><i class="fa fa-search"></i> {__ 'Search'}</a>
			{/if}
		</div>
	{/if}
{/if}
