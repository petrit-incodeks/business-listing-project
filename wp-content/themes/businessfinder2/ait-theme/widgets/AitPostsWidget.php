		...
		if(!empty($posts)){
			...
			foreach($posts as $post){
				...
				if(has_post_thumbnail($post->ID) && $instance['show_thumbnails']){
					...
					$result .= '<span class="thumb-icon"><img class="thumb" style="width: '.$instance['thumbnail_width'].'px;" src="'.aitResizeImage($thumbnail_args['0'], array('width' => $instance['thumbnail_width'], 'height' => $instance['thumbnail_height'], 'crop' => true)).'" alt="'.esc_attr(strip_tags($post->post_title)).'" /></span>';
					$result .= '</div>';
				}
