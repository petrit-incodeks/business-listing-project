...
class AitLoginWidget extends WP_Widget
{
		...
		if( is_user_logged_in() ){
			...
			$result .= '<div class="userlogin-container user-logged-in">';
				...
				$result .= '<div class="profile">';
					...
					$result .= '<div class="profile-name"><span>'.__('Username', 'ait').':</span><span>'.$currentUser->user_login.'</span></div>';
					...
					$result .= '<a href="'.wp_logout_url(get_home_url()).'" title="'.__('Logout','ait').'" class="widgetlogin-button-logout">'.__('Logout','ait').'</a>';
				...
				$result .= '<div class="userlogin-tabs">';
					...
					$result .= '<div class="userlogin-tabs-contents">';
						$result .= '<div class="userlogin-tabs-content '.$openedTabRegister.'">';
							...
							$result .= '<form method="post" action="'.home_url('/').'?ait-action=register&source=widget&number='.$args["widget_id"].'" class="wp-user-form user-register-form">';
								$result .= '<p class="input-container input-username">';
									...
									$result .= ( isset($instance['helper_username']) && $instance['helper_username'] != '' ) ? '<span class="input-helper">'.$instance['helper_username'].'</span>' : '';
								$result .= '</p>';
								$result .= '<p class="input-container input-email">';
									...
									$result .= ( isset($instance['helper_email']) && $instance['helper_email'] != '' ) ? '<span class="input-helper">'.$instance['helper_email'].'</span>' : '';
								$result .= '</p>';
								...
								$result .= '<p class="input-container input-role">';
									if(sizeof($orderedPackages)==1) {
										$package = $themePackages->getPackageBySlug($orderedPackages[0]);
										$packageOptions = $package->getOptions();
										$isFree = $packageOptions['price'] == 0 ? "true" : "false";
										$result .= '<span>'.$package->getName().' ('.$packageOptions['price'].' '.$themeOptions['payments']['currency'].')</span>'.'<input type="hidden" name="user_role" value="'.$package->getSlug().'" data-isfree="'.$isFree.'"/>';
									} else {
										$result .= '<select id="user_role" name="user_role" tabindex="-1">';
								...
								if(count($paymentGatesEnabled) > 0){
									...
									if ($firstPackageOptions['price'] == 0) {
										...
										$result .= '<select id="user_payment" name="user_payment" tabindex="-1">';
								...
								if(class_exists("AitReallySimpleCaptcha")){
									...
									$result .= '<p class="input-container input-captcha">';
										...
										$result .= '<input type="text" name="user_captcha" id="user_captcha" value="" size="20" tabindex="103" />';
									$result .= '</p>';
								}
								/* CAPTCHA VALIDATION */
								...
								$result .= '<div class="login-fields">';
									...
									$result .= '<input type="submit" name="user-submit" value="'.__('Sign up!', 'ait').'" class="user-submit" tabindex="104" />';
						...
						$result .= '<div class="userlogin-tabs-content '.$openedTabLogin.'">';
							...
							$result .= wp_login_form( array( 'redirect' => get_home_url(), 'form_id' => 'ait-login-form-widget', 'id_username' => 'user_login_log', 'echo' => false, 'remember' => false,) );
						$result .= '</div>';
					$result .= '</div>';
				$result .= '</div>';
				echo($result);
	...
	function update($new_instance, $old_instance)
	{
		...
		$instance['helper_username'] = $new_instance['helper_username'];
		$instance['helper_email'] = $new_instance['helper_email'];
    ...
    ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'helper_username' ); ?>"><?php echo __( 'Help text for Username field', 'ait-admin' ); ?>:</label>
			<input type="text" id="<?php echo $this->get_field_id( 'helper_username' ); ?>" name="<?php echo $this->get_field_name( 'helper_username' ); ?>" value="<?php echo htmlspecialchars($instance['helper_username']); ?>"class="widefat" style="width:100%;" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'helper_email' ); ?>"><?php echo __( 'Help text for Email field', 'ait-admin' ); ?>:</label>
			<input type="text" id="<?php echo $this->get_field_id( 'helper_email' ); ?>" name="<?php echo $this->get_field_name( 'helper_email' ); ?>" value="<?php echo htmlspecialchars($instance['helper_email']); ?>"class="widefat" style="width:100%;" />
		</p>
		<p>
