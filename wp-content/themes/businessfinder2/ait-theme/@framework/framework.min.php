...
aitOptions($type=null){if($type)return
AitTheme::getOptions()->get($type);return
...
false;}}function
aitIsGutenbergActive(){global$wp_version;return((defined('GUTENBERG_VERSION')or
version_compare($wp_version,'5.0-rc','>='))and!function_exists('classic_editor_replace'));}function
aitHasBlocks($content){return(function_exists('has_blocks')and
has_blocks($content));}function
...
aitIsOurServer(){return
defined("AIT_SERVER");}if(!function_exists('d')){function
d(){foreach(func_get_args()as$arg){echo"<xmp style='outline:1px solid red;background:ivory;position:relative;z-index:9999;clear:both;'>";var_dump($arg);echo"</xmp>";}}}if(!function_exists('dd')){function
dd(){foreach(func_get_args()as$arg){echo"<xmp style='outline:1px solid red;background:ivory;position:relative;z-index:9999;clear:both;'>";var_dump($arg);echo"</xmp>";}die();}}if(!function_exists('p')){function
p(){foreach(func_get_args()as$arg){echo"<xmp style='outline:1px solid red;background:ivory;position:relative;z-index:9999;clear:both;'>";print_r($arg);echo"</xmp>";}}}if(!function_exists('pd')){function
pd(){foreach(func_get_args()as$arg){echo"<xmp style='outline:1px solid red;background:ivory;position:relative;z-index:9999;clear:both;'>";print_r($arg);echo"</xmp>";}die();}}if(!function_exists('array_replace_recursive')){function
...
getFullConfig($configType=''){if(!isset($this->storage['full-config'])){$r=$this->processConfig($this->getRawConfig());$this->storage['full-config']=$r['full-config'];}if(isset($this->storage['full-config'][$configType]))return
apply_filters('ait-get-full-config',$this->storage['full-config'][$configType],$configType);else{if(empty($configType)){return
apply_filters('ait-get-full-config',$this->storage['full-config'],'');}else{trigger_error("There is no config type '$configType' for ".__METHOD__." method");}}}function
...
aitPaths()->dir->fwConfig.'/admin.php';$configFile=aitPath('config','/admin.neon');$config=self::loadRawConfig($configFile,'/admin.neon',true);$this->storage['admin-config']=array_merge($defaults,$config);}$this->storage['admin-config']=apply_filters('ait-admin-config',$this->storage['admin-config'],$group);if($group){if(isset($this->storage['admin-config'][$group]))return$this->storage['admin-config'][$group];else
return
...
createFullConfig($rawConfig){$fullConfig=array();$j=$sectionIndex=0;foreach($rawConfig
as$configType=>$groups){if($configType=='elements'){$fullConfig['elements']=$this->createElementsFullConfig($groups);}else{foreach($groups
as$groupKey=>$groupValues){$fullConfig[$configType][$groupKey]=$this->convertGroupForFullConfig($groupKey,$groupValues);if(!isset($groupValues['options'])){$fullConfig[$configType][$groupKey]['@options']=array();$groupValues['options']=array();}$hasSections=count(array_filter(array_keys($groupValues['options']),'is_numeric'))>0;if(!$hasSections
...
createElementsFullConfig($elements){$return=array();$row=$cols=$j=$sectionIndex=0;$i=0;foreach($elements
as$elKey=>$elValues){$return[$i]=$this->convertGroupForFullConfig($elKey,$elValues,true);$return[$i]['@element']=$elKey;$hasSections=count(array_filter(array_keys($elValues['options']),'is_numeric'))>0;if(!$hasSections
...
loadMainConfigs(){$f=$this->getMainConfigFiles();$config=array();$config['theme']=$this->loadThemeConfig($f['theme'],$f['theme-built-in']);$config['layout']=$this->loadLayoutConfig($f['layout'],$f['layout-built-in']);$config['elements']=$this->loadElementsConfigs($f['elements'],$f['elements-built-in']);$pluginsConfigTypes=apply_filters('ait-config-types',array());foreach($pluginsConfigTypes
as$key){if(isset($config[$key]))continue;$config[$key]=self::loadRawConfig($f[$key]);}return$config;}function
...
function
getMainConfigTypes(){$configTypes=array('theme','layout','elements');return
apply_filters('ait-config-types',$configTypes);}static
function
...
getGlobalOptions(){$theme=get_option($this->getOptionKey('theme'),array());if(!isset($this->options['theme'])){$this->options['theme']=array();}if(!empty($theme)){$this->options['theme']=$this->mergeConfigDefaultsAndOptions($this->options['theme'],$theme);}$layout=get_option($this->getOptionKey('layout'),array());if(!isset($this->options['layout'])){$this->options['layout']=array();}if(!empty($layout)){$this->options['layout']=$this->mergeConfigDefaultsAndOptions($this->options['layout'],$layout);}$elements=get_option($this->getOptionKey('elements'),null);if(!isset($this->options['elements'])){$this->options['elements']=array();}$fullConfig=$this->config->getFullConfig('elements');if($elements===null){$this->options['elements']=$this->getDefaultsOnlyOfUsedElements($fullConfig);}else{$this->options['elements']=$this->mergeElementsConfigDefaultsAndOptions($this->options['elements'],$elements,$fullConfig);}$pluginConfigs=array();$pluginConfigs=apply_filters('ait-config-types',$pluginConfigs);foreach($pluginConfigs
as$key){if(in_array($key,array('theme','layout','elements')))continue;$plugin=get_option($this->getOptionKey($key),array());if(!isset($this->options[$key])){$this->options[$key]=array();}if(!empty($plugin)){$this->options[$key]=$this->mergeConfigDefaultsAndOptions($this->options[$key],$plugin);}}return$this->options;}function
...
and
self::$frontpage->blog){$blogPage=get_page(self::$frontpage->blog)->post_title;}$pages=array('_blog'=>array('label'=>$blogPage?sprintf(__('%s (blog)','ait-admin'),$blogPage):esc_html__('Homepage (blog)','ait-admin'),'with-id'=>false,'if'=>'is_home'),'_404'=>array('label'=>esc_html__('404 page','ait-admin'),'with-id'=>false,'if'=>'is_404'),'_search'=>array('label'=>esc_html__('Search page','ait-admin'),'with-id'=>false,'if'=>'is_search'),'_archive'=>array('label'=>esc_html__('Archive pages','ait-admin'),'sub-label'=>esc_html__('Category, Taxonomy, Tag, Author, Date','ait-admin'),'with-id'=>false,'if'=>'is_archive'),'_attachment'=>array('label'=>esc_html__('Attachment pages','ait-admin'),'with-id'=>false,'if'=>'is_attachment'),'_post'=>array('label'=>esc_html__('Single post','ait-admin'),'with-id'=>false,'if'=>array('is_singular',array('post'))));if(aitIsPluginActive('toolkit')){$cpts=aitManager('cpts')->getAll();foreach($cpts
as$cpt){if($cpt
instanceof
AitPublicCpt){$pages["_{$cpt->getId()}"]=array('label'=>$cpt->getLabels()->singular_name,'with-id'=>false,'if'=>array('is_singular',array($cpt->getInternalId())));}}}if(AitWoocommerce::enabled()){$pages['_wc_product']=array('label'=>esc_html__('Single - WooCommerce Product','ait-admin'),'with-id'=>false,'if'=>array('AitWoocommerce::currentPageIs',array('product')));$shop=AitWoocommerce::getPage('shop');if($shop){$pages['_wc_shop']=array('label'=>sprintf(__('%s - WooCommerce Shop Page','ait-admin'),$shop->post_title),'with-id'=>false,'if'=>array('AitWoocommerce::currentPageIs',array('woocommerce')));}}self::$specialPages=apply_filters('ait-special-custom-pages',$pages);return
...
as$oid=>$values){$ifResult=false;if(is_callable($values['if'])){$ifResult=call_user_func($values['if']);}elseif(is_string($values['if'])and
is_callable($values['if'])){$ifResult=call_user_func($values['if']);}elseif(is_array($values['if'])and
is_string($values['if'][0])and
is_callable($values['if'][0])and
is_array($values['if'][1])){$ifResult=call_user_func_array($values['if'][0],$values['if'][1]);}if($ifResult){$key=$oid;$id=get_queried_object_id();if($id
...
getRequestedPluginCodename($requestMethod='post'){$codename='';if($requestMethod=='post'){$codename=isset($_POST['pluginCodename'])?$_POST['pluginCodename']:'';}elseif($requestMethod=='get'){$codename=isset($_GET['pluginCodename'])?$_GET['pluginCodename']:'';}return
sanitize_key($codename);}function
...
getContentPreviewDefaultOptions(){$layout=$this->option('layout');$rows=$this->option($layout.'Rows');$columns=$this->option($layout.'Columns');$carousel=$this->option($layout.'EnableCarousel');return
array('layout'=>$layout,'columns'=>(!empty($columns)?$columns:1),'carousel'=>$carousel,'rows'=>(!empty($rows)?($carousel?$rows:1):1),'content'=>true,'script'=>true);}function
getContentPreviewOptions(){return
array();}function
getContentPreview($elementData=array()){$defaultContentPreviewOptions=$this->getContentPreviewDefaultOptions();$contentPreviewOptions=$this->getContentPreviewOptions();$options=array_merge($defaultContentPreviewOptions,$contentPreviewOptions);$elementData['options']=$options;if($options['layout']===false||$options['columns']===false)return
null;ob_start();?>
		...
		<?php
$content=ob_get_clean();if($options['script']):$elementName=aitPath('elements',"/{$this->id}/admin/element-preview.js")?$this->id:'@common';ob_start();?>

			<script type=<?php echo($this->isUsed()?"text/javascript":"text/template")?>>
				(function(){
					var elementData = <?php echo
json_encode($elementData);?>;
					<?php echo
file_get_contents(aitPath('elements',"/{$elementName}/admin/element-preview.js"));?>
				})();
			</script>

			<?php
$script=ob_get_clean();else:$script=false;endif;$preview=array('content'=>$content,'script'=>$script);return$preview;}function
getTitle(){return$this->title;}function
getIcon(){return$this->icon;}function
...
as$key=>$value){if($value==true){$layout=substr($key,0,strpos($key,'EnableCarousel'));$layoutOptions=$this->findOptionsContaining("[$layout]");if(count($layoutOptions)==1&&reset($layoutOptions)==true){$classes[]='carousel-enabled';}}}$this->addClassesFromSelectInputTypes($classes);if(AIT_THEME_PACKAGE==='basic'){$classes[]='load-finished';}$classes=array_unique($classes);return$asString?implode(' ',$classes):$classes;}function
...
getJsObject(){$o=array('defaults'=>$this->optionsDefaults,'current'=>$this->options,'paths'=>$this->getPaths()->url);$var='var '.$this->getJsObjectName()." = ".json_encode($o).';';return$var;}function
...
isEnabled(){$return=true;if($this->config['@disabled']===false){if(!$this->isSortable()){$return=true;}elseif($this->hasAllCptsEnabled()&&aitIsPluginActive('toolkit')){$return=true;}else{$return=false;}}else{$return=false;}return$return;}function
...
function
getCurrentLocaleText($localesAndTexts,$defaultText=''){$currentLocale=self::getCurrentLocale();if(is_admin()){$post=self::checkIfPostAndGetLang();if($post){$currentLocale=$post->locale;}}$return=$defaultText;if(is_array($localesAndTexts)and
...
isset($localesAndTexts->{'en_US'})){$return=$localesAndTexts->{'en_US'};}elseif(is_string($localesAndTexts)and!empty($localesAndTexts)){$return=$localesAndTexts;}return$return;}static
function
...
function
getGmapsLang(){$map=array('bg_BG'=>'bg','cs_CZ'=>'cs','de_DE'=>'de','de_CH'=>'de','el'=>'el','en_US'=>'en','en_CA'=>'en-ca','en_GB'=>'en-gb','es_ES'=>'es','es_CL'=>'es-cl','es_AR'=>'es-ar','es_CL'=>'es-cl','es_CO'=>'es-co','es_GT'=>'es-gt','es_MX'=>'es-mx','es_PE'=>'es-pe','es_PR'=>'es-pr','es_VE'=>'es-ve','fi'=>'fi','fr_FR'=>'fr','fr_BE'=>'fr','fr_CA'=>'fr','hi_IN'=>'hi','hr'=>'hr','hu_HU'=>'hu','id_ID'=>'id','it_IT'=>'it','nl_NL'=>'nl','pl_PL'=>'pl','pt_BR'=>'pt-br','pt_PT'=>'pt-pt','ru_RU'=>'ru','sk_SK'=>'sk','sq'=>'sq','sv_SE'=>'sv','tr_TR'=>'tr','uk'=>'uk','zh_CN'=>'zh-cn','zh_TW'=>'zh-tw');$currentLocale=self::getCurrentLocale();if(isset($map[$currentLocale])){return$map[$currentLocale];}return'en';}static
function
getFullCalendarLocale(){$lang=self::getGmapsLang();return
str_replace(array('pt-pt'),array('pt'),$lang);}}class
...
as$key){if(!isset($_POST['menu-item-'.$key][$menu_item_db])){$_POST['menu-item-'.$key][$menu_item_db]="";}$value=$_POST['menu-item-'.$key][$menu_item_db];update_post_meta($menu_item_db,'_menu-item-'.$key,$value);}}}final
class
AitTheme
extends
NObject{private
static$alreadyRan=false;protected
static$configuration=array('frontend-ajax'=>array(),'menus'=>array(),'theme-support'=>array('html5'=>array('search-form','comment-form','comment-list'),'wplatte'),'ait-theme-support'=>array(),'sidebars'=>array(),'page-post-metaboxes'=>array(),'builtin-assets'=>array(),'assets'=>array(),'plugins'=>array(),'paid-plugins'=>array(),'editor-style'=>true,'widget-output-cache'=>0);protected
...
self::$managers[$manager];}else{trigger_error(sprintf("Manager '{$manager}' does not exist. Available managers are: %s",implode(', ',array_keys(self::$managers))),E_USER_WARNING);}return
false;}static
function
getFactory($factory){if(isset(self::$factories[$factory])){return
self::$factories[$factory];}else{trigger_error(sprintf("Factory '{$factory}' does not exist. Available factories are: %s",implode(', ',array_keys(self::$factories))),E_USER_WARNING);}return
false;}static
function
...
static
function
preparePluginsConfigration(){$builtinPluginList=require
aitPaths()->dir->fwConfig.'/plugins.php';$plugins=apply_filters('ait-builtin-plugin-list',array_replace_recursive($builtinPluginList,self::$configuration['plugins']));$list=array('plugins'=>array(),'paid-plugins'=>array());foreach($plugins
as$slug=>$plugin){if(isset($plugin['ait-packages'])and!in_array('basic',$plugin['ait-packages'])){$list['paid-plugins'][$slug]=$plugin;$list['paid-plugins'][$slug]['slug']=$slug;}else{$list['plugins'][$slug]=$plugin;$list['plugins'][$slug]['slug']=$slug;}if($slug==='revslider'){$revsliderInstalled=file_exists(WP_PLUGIN_DIR."/revslider/");$aitFileDoesNotExist=!file_exists(WP_PLUGIN_DIR."/revslider/ait-revslider.php");if($revsliderInstalled
and$aitFileDoesNotExist)continue;}if(isset($plugin['source'])and
AitUtils::startsWith($plugin['source'],'/plugins/')){$list['paid-plugins'][$slug]['source']=aitPaths()->dir->ait.$plugin['source'];$list['plugins'][$slug]['source']=aitPaths()->dir->ait.$plugin['source'];}add_theme_support($slug.'-plugin');}self::$configuration['plugins']=$list['plugins'];self::$configuration['paid-plugins']=$list['paid-plugins'];add_theme_support('ait-languages-plugin');}protected
static
function
tgmpa(){require_once
...
function
tgmpaRegisterPlugins(){$config=array('id'=>'ait-tgmpa','parent_slug'=>'plugins.php','menu'=>'install-required-plugins','is_automatic'=>true,'strings'=>array('menu_title'=>__('Install Theme Plugins','ait-admin'),'page_title'=>__('Install Plugins Recommended / Required by Theme','ait-admin')));if(!empty(self::$configuration['plugins'])or!empty(self::$configuration['paid-plugins'])){$plugins=self::$configuration['plugins'];if(!empty(self::$configuration['paid-plugins'])and
AIT_THEME_PACKAGE!=='basic'){$plugins=array_merge($plugins,self::$configuration['paid-plugins']);}tgmpa($plugins,$config);}}static
function
...
function
overrideCptMetaboxConfig($path,$filename){$paths=aitPaths();$newPath='';if(AitUtils::endsWith($filename,'.php')){$basename=basename($filename,'.php');$newPath=aitPath('config',"/cpts-metaboxes/{$basename}.php");if(!$newPath){$newPath=aitPath('config',"/cpts-metaboxes/{$basename}.neon");}}else{$newPath=aitPath('config',"/cpts-metaboxes/{$filename}");}return$newPath?$newPath:$path;}}class
AitWpLatteMacros
extends
NMacroSet{public
...
macroIncludeElement(NMacroNode$node,NPhpWriter$writer){$el=$writer->formatArgs();$params=array("'el' => $el","'element' => {$el}","'htmlId' => {$el}->getHtmlId()","'htmlClass' => {$el}->getHtmlClass()");$paramsStr=implode(', ',$params);$include=$writer->write('NCoreMacros::includeTemplate('.$el.'->getTemplate(), array('.$paramsStr.') + $template->getParameters(), $_l->templates[%var])',$this->getCompiler()->getTemplateId());if($node->modifiers){$includeElement=$writer->write('echo %modify(%raw->__toString(TRUE))',$include);}else{$includeElement=$include.'->render()';}$c=__CLASS__;return"if({$el}->getId() === 'columns' or {$el}->isEnabled()){
				{$includeElement};
			}else{
				echo {$c}::elementPlaceholder({$el});
			}";}static
function
elementPlaceholder($element){$elementColor=$element->getColor();$maxWidth=isset(aitOptions()->get('theme')->general->websiteWidth)?aitOptions()->get('theme')->general->websiteWidth:'';$maxWidthStyle=is_numeric($maxWidth)&&$maxWidth>500?"style='max-width: {$maxWidth}px'":"";return"
				...
				<div class='ait-elm-placeholder'>
					...
					<div class='ait-elm-placeholder-content'>
						<div class='ait-elm-placeholder-text'>
							<h2 style='color: {$elementColor}'>".sprintf(__('%s Element','ait'),$element->title)."</h2>"."<div>".sprintf(__('%1$s is available in %2$s plugin','ait'),'','AIT Elements Toolkit')."</div>
						</div>
...
function
googleAnalytics($uaCode='',$anonymizeIp=false){if($uaCode){$aip=apply_filters('ait-ga-anonymize-ip',$anonymizeIp)?"ga('set', 'anonymizeIp', true);":'';return"
<script>
...
function
currency($price=0,$currency='USD'){$dollar='&#36;';$krone='&#107;&#114;';$currencies=array('AUD'=>array('symbol'=>$dollar),'BRL'=>array('symbol'=>'&#82;'.$dollar),'CAD'=>array('symbol'=>$dollar),'CZK'=>array('symbol'=>'&#75;&#269;','position'=>'right'),'DKK'=>array('symbol'=>$krone),'EUR'=>array('symbol'=>'&nbsp;&#8364;','position'=>'right'),'HKD'=>array('symbol'=>$dollar),'HUF'=>array('symbol'=>'&#70;&#116;'),'ILS'=>array('symbol'=>'&#8362;'),'JPY'=>array('symbol'=>'&#165;'),'MYR'=>array('symbol'=>'&#82;&#77;'),'MXN'=>array('symbol'=>$dollar),'NOK'=>array('symbol'=>$krone),'NZD'=>array('symbol'=>$dollar),'PHP'=>array('symbol'=>'&#8369;'),'PLN'=>array('symbol'=>'&#122;&#322;'),'GBP'=>array('symbol'=>'&#163;'),'RUB'=>array('symbol'=>'&nbsp;&#1088;&#1091;&#1073;','position'=>'right'),'SGD'=>array('symbol'=>$dollar),'SEK'=>array('symbol'=>$krone),'CHF'=>array('symbol'=>'&#67;&#72;&#70;'),'TWD'=>array('symbol'=>'&#78;&#84;'.$dollar),'THB'=>array('symbol'=>'&#3647;'),'TRY'=>array('symbol'=>'&#8378;'),'USD'=>array('symbol'=>$dollar));$priceLayout="<span class='price' data-raw='%d'>%s</span>";$currencyLayout="<span class='currency'>%s</span>";$formattedPrice=number_format_i18n($price,2);if(!isset($currencies[$currency])){trigger_error("Currency $currency is not supported");$return=sprintf($currencyLayout,$dollar).sprintf($priceLayout,$price,$formattedPrice);}$c=$currencies[$currency];if(isset($c['position'])and$c['position']=='right'){$return=sprintf($priceLayout,$price,$formattedPrice).sprintf($currencyLayout,$c['symbol']);}else{$return=sprintf($currencyLayout,$c['symbol']).sprintf($priceLayout,$price,$formattedPrice);}return
...
function
enableSearchingInElements($where,$wp_query){if(!$wp_query->is_search()||basename($_SERVER["SCRIPT_NAME"])=="admin-ajax.php"||(isset($_REQUEST['a'])&&$_REQUEST['a']==true)){return$where;}$searchQuery=self::buildDefaultSearchSql($wp_query);$searchQuery.=self::buildPostElementsOptionsSearchSql($wp_query);if($searchQuery!=''){$where=preg_replace('#\(\(\(.*?\)\)\)#','(('.$searchQuery.'))',$where);}return$where;}static
function
joinPostsMetadataToEnableSearchingInElements($join){global$wpdb;if(is_search()&&!isset($_REQUEST['a'])){$theme=esc_sql(AIT_CURRENT_THEME);$join.=" LEFT JOIN {$wpdb->options} ON {$wpdb->options}.option_name LIKE CONCAT('_ait_{$theme}_elements_opts_page_', {$wpdb->posts}.ID)";}return$join;}static
function
...
static
function
getSearchTerms($wp_query){$s=isset($wp_query->query_vars['s'])?$wp_query->query_vars['s']:'';$sentence=isset($wp_query->query_vars['sentence'])?$wp_query->query_vars['sentence']:false;$search_terms=array();if(!empty($s)){$s=stripslashes($s);if($sentence){$search_terms=array($s);}else{preg_match_all('/".*?("|$)|((?<=[\\s",+])|^)[^\\s",+]+/',$s,$matches);$search_terms=array_map(function($a){return
...
as$i=>$params){$realId='__'.trim("{$group}-{$i}",'@');$p=array_diff_assoc($params,$defaults);if($group=='@sidebar'){$this->sidebars[$realId]=$p;}else{$this->widgetAreas[$group][$realId]=$p;}$s=array_merge(array('id'=>$realId),$defaults,$params);$name=AitLangs::getCurrentLocaleText($params['name'],'[not translated sidebar name]');if(empty($name)){$s['name']='[not translated sidebar name]';}else{$s['name']=$name;}$this->dynamicSidebars[]=$s;}}}function
...
static
function
getFullCacheKey($cacheKey){global$wp_version;$cacheKey.=$wp_version.AIT_CURRENT_THEME.AIT_THEME_VERSION.AIT_SKELETON_VERSION;return$cacheKey;}}class
...
as$i=>$elementOptions){$elId=key($elementOptions);if(!current_theme_supports("ait-element-{$elId}"))continue;try{$elements[$i]=$this->createElement($elId,$i,$oid,$elementOptions[$elId]);if($onFrontend){if(($elements[$i]->isEnabled()or
AIT_THEME_PACKAGE==='basic')and$elements[$i]->isDisplay()){$this->assetsManager->addAssets($elements[$i]->getFrontendAssets(),array('paths'=>$elements[$i]->getPaths()));$this->assetsManager->addInlineStyleCallback(array(&$elements[$i],'getInlineStyle'));}}}catch(Exception$e){}}return$elements;}function
...
as$i=>$el){if(!$em->getPrototype($el->getId())and!$em->isElementSidebarsBoundary($el->getId()))continue;if($el->isDisabled()and
AIT_THEME_PACKAGE!=='basic')continue;if($em->isElementSidebarsBoundary($el->getId())and(!self::hasSidebar(__CLASS__,'left')&&!self::hasSidebar(__CLASS__,'right')))continue;if($el->getId()==='comments'and
aitOptions()->isQueryForSpecialPage(array('_404','_search','_archive','_wc_product','_wc_shop')))continue;if($el->sortable
and!aitOptions()->isQueryForSpecialPage()and
post_password_required()and$el->getId()==='content'){$return->sortable=array();$return->sortable[$el->getId()]=$el;break;}if(!$el->sortable){$return->unsortable[$el->getId()]=$el;}elseif(isset($options[$i][$el->getId()]['@columns-element-index'])and$options[$i][$el->getId()]['@columns-element-index']!=''and
...
is_array($metaDefaults[$metaboxKey]))$postmeta=array();$postmeta=array_replace_recursive($metaDefaults[$metaboxKey],$postmeta);$postmeta=apply_filters('ait-wplatte-post-meta',$postmeta,$cpt,$cptId,$metaboxKey,$key,$metaDefaults);array_walk_recursive($postmeta,array(__CLASS__,'filterPostmetaValue'));}}return$postmeta;}static
function
filterPostmetaValue(&$item,$key){$item=apply_filters('ait-filter-value-for-current-locale',$item,$key);}static
function
...
function
register(){$ext=self::loadConfig();if(!AitUtils::isAjax()){add_filter('wp_list_categories',array(__CLASS__,'removeCategoryRelAtt'));add_filter('the_category',array(__CLASS__,'removeCategoryRelAtt'));if(AIT_THEME_PACKAGE==='basic'){add_filter('ait-templates-options',function($options){if(isset($options['theme']['footer']['text'])){$text=sprintf(base64_decode('UG93ZXJlZCBieSAlcyBXb3JkUHJlc3MgdGhlbWUgZnJvbSA8YSBocmVmPSJodHRwczovL3d3dy5haXQtdGhlbWVzLmNsdWIvIj5BaXRUaGVtZXMuY2x1YjwvYT4='),wp_get_theme()->name);if(is_array($options['theme']['footer']['text'])){foreach($options['theme']['footer']['text']as$locale=>$value){$options['theme']['footer']['text'][$locale]=$text;}}else{$options['theme']['footer']['text']=$text;}return$options;}return$options;});add_filter('ait-get-full-config',function($config,$type){if($type==='theme'){foreach($config['footer']['@options']as$i=>$v){if(isset($v['text'])){unset($config['footer']['@options'][$i]['text']);return$config;}}return$config;}return$config;},10,2);}if($ext->wp->loginPageBranding){global$pagenow;if($pagenow=='wp-login.php'){add_action('login_head',array(__CLASS__,'loginPageBranding'));add_filter('login_headerurl',array(__CLASS__,'loginPageBranding'));add_filter('login_headertext',array(__CLASS__,'loginPageBranding'));}}add_filter('http_request_args',array(__CLASS__,'excludeActiveAitThemeFromWpOrgUpdateCheck'),10,2);}add_filter('widget_text','do_shortcode');add_filter('sanitize_user',array(__CLASS__,'allowCyrillicUsernames'),10,3);}static
function
...
isset($branding["loginScreenLogoLink"])and$branding["loginScreenLogoLink"]){return$branding["loginScreenLogoLink"];}if(current_filter()=='login_headertext'and
isset($branding["loginScreenLogoTooltip"])and$branding["loginScreenLogoTooltip"]){return
AitLangs::getCurrentLocaleText($branding["loginScreenLogoTooltip"],$input);}return$input;}static
function
allowCyrillicUsernames($username,$raw_username,$strict){$username=wp_strip_all_tags($raw_username);$username=remove_accents($username);$username=preg_replace('|%([a-fA-F0-9][a-fA-F0-9])|','',$username);$username=preg_replace('/&.+?;/','',$username);if($strict)$username=preg_replace('|[^a-zа-я0-9 _.\-@]|iu','',$username);$username=trim($username);$username=preg_replace('|\s+|',' ',$username);return$username;}static
function
excludeActiveAitThemeFromWpOrgUpdateCheck($args,$url){if($url==='https://api.wordpress.org/themes/update-check/1.1/'){$body=json_decode($args['body']['themes']);$themes=(array)$body->themes;if(isset($themes[AIT_THEME_CODENAME])){unset($themes[AIT_THEME_CODENAME]);}$body->themes=$themes;$args['body']['themes']=wp_json_encode($body);return$args;}return$args;}}class
...
AitSkeletonUpgrade2913;$this->errors=$upgrade->execute();}if(version_compare(self::getSkeletonVersion(),'2.23.0','<')){$upgrade=new
AitSkeletonUpgrade2230;$this->errors=$upgrade->execute();}if(version_compare(self::getSkeletonVersion(),'2.23.30','<')){$upgrade=new
AitSkeletonUpgrade22330;$this->errors=$upgrade->execute();}do_action('ait-skeleton-upgrade',$this);$this->addAdminErrorNotices();}function
...
as$handler=>$css){$lastHandler=$handler;if($css===true){wp_enqueue_style($handler);}elseif(is_array($css)){if($css['file']===false)continue;if(AitUtils::isExtUrl($css['file'])or
...
function
trimHtmlContent($s,$limit,$ellipsis=null){$length=0;$ellipsis=isset($ellipsis)?$ellipsis:' [...]';$tags=array();for($i=0;$i<strlen($s)&&$length<$limit;$i++){switch($s[$i]){case'<':$start=$i+1;while($i<strlen($s)&&$s[$i]!='>'&&!ctype_space($s[$i])){$i++;}$tag=substr($s,$start,$i-$start);$in_quote='';while($i<strlen($s)&&($in_quote||$s[$i]!='>')){if(($s[$i]=='"'||$s[$i]=="'")&&!$in_quote){$in_quote=$s[$i];}elseif($in_quote==$s[$i]){$in_quote='';}$i++;}if($s[$start]=='/'){array_shift($tags);}elseif($s[$i-1]!='/'){array_unshift($tags,$tag);}break;case'&':$length++;while($i<strlen($s)&&$s[$i]!=';'){$i++;}break;default:$length++;while($i+1<strlen($s)&&ord($s[$i+1])>127&&ord($s[$i+1])<192){$i++;}}}$s=substr($s,0,$i);(strlen($s)>$limit)?$s.=$ellipsis:'';if($tags){$s.="</".implode("></",$tags).">";}return$s;}static
function
...
function
getPageId($page){if(!self::enabled())return-1;$fn=function_exists('wc_get_page_id')?'wc_get_page_id':'woocommerce_get_page_id';return$fn($page);}static
function
...
function
accountUrl(){if(!self::enabled())return'#';$fn=function_exists('wc_get_page_id')?'wc_get_page_id':'woocommerce_get_page_id';return
get_permalink($fn('myaccount'));}static
function
...
getHtml(){$hidden='';if(isset($this->config->displayOnlyIf)and
is_callable($this->config->displayOnlyIf)){$result=call_user_func($this->config->displayOnlyIf);if(!$result){$hidden=' style="display:none;" ';}}ob_start();if($this->capabilityEnable){if(current_user_can($this->getCapabilityName())){?>
...
labelWrapper($subKey='',$helpPosition=null){$position=$this->helpPosition();$helpPosition=(isset($helpPosition)and
empty($position))?$helpPosition:$this->helpPosition();ob_start();$this->help();$help=ob_get_clean();?>
		<div class="ait-label-wrapper">
...
function
lessVarHelp(){if(!defined('AIT_SERVER'))return;$configName=$this->parentSection->getParentGroup()->getConfigName();$groupId=$this->parentSection->getParentGroup()->getId();if($this->isLessVar()and$groupId!='adminBranding'and
...
function
helpPosition(){return
isset($this->config->helpPosition)?$this->config->helpPosition:false;}protected
function
control(){?>
		<div class="ait-opt-label">
...
empty($locale)and
isset($value[AitLangs::getDefaultLocale()])){$value=$value[AitLangs::getDefaultLocale()];}if(is_array($value)and
count($value)==1){$value=reset($value);}return$value;}static
function
...
if($this->helpPosition()=='inline'){$this->help();}?>
		</div>
	<?php
}private
function
...
init(){$this->isCloneable=true;}protected
function
control(){$value=$this->getValue();if(is_array($value)){$value=reset($value);}?>
...
is_numeric($label)and!is_string($label)){$input=$label;}if(is_array($val)){if($this->isMulti()){$value=in_array($input,$val)?$input:false;}else{$value='';}}else{$value=$val;}?>
...
init(){$this->isCloneable=true;}protected
function
control(){$val=$this->getValue();if(isset($this->config->cpt)and$this->config->cpt){$cptName=AitUtils::addPrefix($this->config->cpt,'post');}else{$cptName='post';}$isMultilingual=(isset($this->config->multilingual)and$this->config->multilingual)?$this->config->multilingual:false;if(post_type_exists($cptName)){$cpt=get_post_type_object($cptName);$args=array('post_type'=>$cptName,'selected'=>$val,'show_option_none'=>__('&mdash; Select &mdash;','default'),'option_none_value'=>'0','posts_per_page'=>-1,'post_status'=>array('publish','draft'));if(!$cpt->hierarchical){$args['hierarchical']=false;}if(isset($this->config->showCurrentUserPosts)){if($this->config->showCurrentUserPosts=true){global$current_user;if(!in_array("administrator",$current_user->roles)){$excluded=array();$posts=new
WP_Query(array('post_type'=>$cptName,'posts_per_page'=>-1,'author__not_in'=>array($current_user->ID)));foreach($posts->posts
as$post){array_push($excluded,$post->ID);}$args['exclude']=join(',',$excluded);}}}$argsFromConfig=(isset($this->config->args)and!empty($this->config->args))?$this->config->args:array();$args=array_merge($args,$argsFromConfig,array('echo'=>false,'name'=>$this->getNameAttr(),'id'=>$this->getIdAttr(),'class'=>$this->id.' chosen'));if($isMultilingual&&AitLangs::isEnabled()){if($this->config->label=='@native')$this->config->label=$cpt->labels->singular_name;?>
				<div class="ait-opt-label">
					<?php $this->labelWrapper()?>
				</div>
...
foreach(AitLangs::getLanguagesList()as$lang){$args['lang']=$lang->slug;$args['name']=$this->getLocalisedNameAttr('',$lang->locale);$args['id']=$this->getLocalisedIdAttr('',$lang->locale);$args['selected']=$this->getLocalisedValue('',$lang->locale);if(!AitLangs::isFilteredOut($lang)){$dropdown='';$emptySelect=function($output)use($cpt,$args){return
sprintf(empty($output)?"<select data-placeholder='%s' name='%s' id='%s' class='%s'></select>":$output,esc_attr(sprintf(__('No items. Add some items to "%s"','ait-admin'),$cpt->labels->menu_name)),$args['name'],$args['id'],$args['class']);};add_filter('wp_dropdown_pages',$emptySelect);add_filter('ait-dropdown-posts',$emptySelect);if($cpt->hierarchical){$dropdown=wp_dropdown_pages($args);$dropdown=str_replace('<select',"<select class='{$args['class']}'",$dropdown);}else{$dropdown=aitDropdownPosts($args);}remove_filter('wp_dropdown_pages',$emptySelect);remove_filter('ait-dropdown-posts',$emptySelect);?>
							<div class="ait-opt-wrapper chosen-wrapper ait-langs-enabled <?php echo
AitLangs::htmlClass($lang->locale)?>">
								<?php
echo"<div class=\"flag\">{$lang->flag}</div>";echo$dropdown;?>
							</div>
...
if($this->helpPosition()=='inline'):?>
				...
				<?php
endif;}else{$dropdown='';$emptySelect=function($output)use($cpt,$args){return
...
esc_attr($this->getValue());?>">
					<a href="#" class="datepicker-reset" style="position: absolute; top: 5px; right: 47px" onclick="javascript: event.preventDefault(); jQuery(this).parent().find('input[type=text], input[type=hidden]').attr('value', ''); return false;"><i class="fa fa-times"></i></a>
				</div>
			</div>
		</div>
...
if($this->helpPosition()!='label'){$this->help();}?>
		</div>
...
control(){?>
		...
		<?php
$labelText=$this->getLabelText();ob_start();$this->help();$help=ob_get_clean();$emptyLabelWrapper=(empty($labelText)and
empty($help))?:false;?>
...
if(is_array($icons)&&!empty($icons)){usort($icons,function($a,$b){return
strcasecmp($a->name,$b->name);});foreach($icons
...
as$type=>$params):?>
								<optgroup label="<?php echo($params['label'])?>">
									<?php if($type!='google'):?><option <?php selected($fontType,$type)?> value="<?php echo"{$type}@{$params['font-family']}"?>"><?php echo
esc_html($params['label']);?></option><?php

else:foreach($fonts
as$font):?><option <?php selected($selectedFontType,"google@{$font->family}")?> value="<?php echo"google@{$font->family}"?>" ><?php echo
esc_html($font->family)?></option><?php
...
AitOptionControl{protected
function
init(){$this->key='@enabledAdvanced';$this->config=(object)array('label'=>__('Enable','ait-admin'),'default'=>'0','help'=>__('Enable to override advanced options from Default Layout','ait-admin'));}protected
function
...
has_filter('mce_buttons',array('AitShortcodesGenerator','addMceButtons'))){remove_filter('mce_buttons',array('AitShortcodesGenerator','addMceButtons'));}?>
		...
		<?php endif;?>
			...
			<?php if(aitIsGutenbergActive()or
aitHasBlocks($postContent)):?>
				<div class="ait-opt">
					<a href="<?php echo
get_edit_post_link($post->ID)?>" target="_blank" class="button button-primary"><?php _e('Edit page content &raquo;','ait');?></a>
				</div>
			<?php else:?>
...
esc_attr($this->getStep())?>" data-initval="<?php echo
esc_attr($this->getValue())?>" value="<?php echo
esc_attr($this->getValue())?>">
				<?php if($this->getUnit()!=''):?>
				<span class="ait-unit ait-number-unit"><?php echo
...
AitMapOptionControl
extends
AitOptionControl{protected$mapProvider='google';protected
function
init(){$this->isLessVar=false;$this->mapProvider=$this->useOpenstreetmap()?'openstreetmap':'google';}protected
function
control(){$d=(object)$this->config->default;?>
		<div class="ait-opt-label">
			<?php $this->labelWrapper()?>
					...
					<div class="ait-opt-tools-row">
						<div class="ait-opt-tools-cell1">
							...
							<div class="ait-opt-maps-item ait-opt-maps-address">
							 ...
							 --><div class="ait-control-wrapper">
							 		<?php
$data=$this->getValue();if(is_string($data)){$val=$data;}else{if(!isset($data['address'])){$val=AitLangs::getCurrentLocaleText($data);}else{$val=$data['address'];}}?>
...
function
prepareDefaultValue($optionControlDefinition){$d=array('address'=>'','latitude'=>1,'longitude'=>1,'streetview'=>false,'swheading'=>90,'swpitch'=>5,'swzoom'=>1);$d=array_merge($d,$optionControlDefinition['default']);return$d;}protected
function
useOpenstreetmap(){$theme=wp_get_theme();$currentTheme=$theme->parent()!=false?$theme->parent()->stylesheet:$theme->stylesheet;$isDirectory2=$currentTheme=='skeleton'?$theme->stylesheet=='directory2':$currentTheme=='directory2';$themeOptions=aitOptions()->get('theme');return($isDirectory2&&!$themeOptions->google->mapsApiKey);}}class
...
function
addPageBuilderLink($wp_admin_bar){global$typenow;global$pagenow;global$post;if(!current_user_can(apply_filters('ait-admin-bar-permission','manage_options')))return;$oid=aitOptions()->getOid();$args=array('page'=>'pages-options','oid'=>$oid,'oidnonce'=>AitUtils::nonce('oidnonce'));if(isset($post)and$pagenow=='post.php'){$args['oid']='_page_'.$post->ID;$b=aitOptions()->getFrontpage();if($b->customFrontpage
and$b->blog==$post->ID){$args['oid']='_blog';}}__('Page Builder','ait');if(!is_admin()or(is_admin()and$pagenow=='post.php'and$typenow=='page')){$wp_admin_bar->add_node(array('id'=>'page-builder','title'=>__('Page Builder','ait-admin'),'href'=>AitUtils::adminPageUrl($args)));}}static
function
...
function
ajaxEmptyWPThumbCacheDir(){AitUtils::checkAjaxNonce('delete-cache-image');$u=WP_Thumb::uploadDir();$path=$u['basedir'].'/cache/images';AitUtils::delete($path,"*");wp_send_json_success();}}