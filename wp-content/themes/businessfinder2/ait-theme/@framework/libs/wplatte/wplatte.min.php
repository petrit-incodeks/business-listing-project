...
define('WPLATTE_VERSION','2.9.2');class
WpLatte{public
...
function
ngettextWithContextCallback($translated,$single,$plural,$number,$context,$domain){if($domain=='wplatte'){$translated=self::$translationsForDomain->translate_plural($single,$plural,$number,$context);}return$translated;}protected
static
function
fixSignupPage(){add_action('wp',function(){ob_start();},2000);add_action('before_signup_form',array(__CLASS__,'fixHeaderOnSignupPage'),0);add_action('after_signup_form',function(){ob_start();},0);add_action('shutdown',array(__CLASS__,'fixFooterOnSignupPage'),0);}static
function
...
self::removeRawIncludedContent(did_action('get_footer'),$footerHtml,'footer');}static
function
homeUrl($isSearch=false){global$polylang;if(function_exists('PLL')){return!empty(PLL()->links)?PLL()->links->get_home_url(AitLangs::getCurrentLang(),$isSearch):home_url('/');}if(isset($polylang)){return!empty($polylang->links)?$polylang->links->get_home_url(AitLangs::getCurrentLang(),$isSearch):home_url('/');}return
home_url('/');}}class
WpLatteBaseEntity
extends
...
post_type_archive_title('',false);}else{return'';}}}class
...
willCommentsPaginate(){return(get_comment_pages_count()>1
and
...
as$post_type_object){if($taxonomy->rewrite['slug']===$post_type_object->has_archive){$label=!empty($post_type_object->labels->archive_title)?$post_type_object->labels->archive_title:$post_type_object->labels->name;$trail[]='<a href="'.get_post_type_archive_link($post_type_object->name).'" title="'.esc_attr($label).'">'.$label.'</a>';break;}}}}if(is_taxonomy_hierarchical($term->taxonomy)&&$term->parent)$trail=array_merge($trail,self::getTermParents($term->parent,$term->taxonomy));if(is_paged())$trail[]='<a href="'.esc_url(get_term_link($term,$term->taxonomy)).'" title="'.esc_attr(single_term_title('',false)).'">'.single_term_title('',false).'</a>';else$trail[]=single_term_title('',false);}elseif(is_post_type_archive()){$post_type_object=get_post_type_object(get_query_var('post_type'));if(isset($post_type_object->rewrite['with_front'])&&$post_type_object->rewrite['with_front']&&$wp_rewrite->front)$path.=trailingslashit($wp_rewrite->front);if(!empty($post_type_object->rewrite['slug']))$path.=$post_type_object->rewrite['slug'];if(!empty($path))$trail=array_merge($trail,self::getParents('',$path));if(is_paged())$trail[]='<a href="'.esc_url(get_post_type_archive_link($post_type_object->name)).'" title="'.esc_attr(post_type_archive_title('',false)).'">'.post_type_archive_title('',false).'</a>';else$trail[]=post_type_archive_title('',false);}elseif(is_author()){$user_id=get_query_var('author');if(!empty($wp_rewrite->front))$path.=trailingslashit($wp_rewrite->front);if(!empty($wp_rewrite->author_base))$path.=$wp_rewrite->author_base;if(!empty($path))$trail=array_merge($trail,self::getParents('',$path));if(is_paged())$trail[]='<a href="'.esc_url(get_author_posts_url($user_id)).'" title="'.esc_attr(get_the_author_meta('display_name',$user_id)).'">'.__('Author:','wplatte').' '.get_the_author_meta('display_name',$user_id).'</a>';else$trail[]=__('Author:','wplatte').' '.get_the_author_meta('display_name',$user_id);}elseif(is_time()){if(get_query_var('minute')&&get_query_var('hour'))$trail[]=get_the_time(__('g:i a','wplatte'));elseif(get_query_var('minute'))$trail[]=sprintf(__('Minute %1$s','wplatte'),get_the_time(__('i','wplatte')));elseif(get_query_var('hour'))$trail[]=get_the_time(esc_html_x('g a','hour archives time format','wplatte'));}elseif(is_date()){if($wp_rewrite->front)$trail=array_merge($trail,self::getParents('',$wp_rewrite->front));if(is_day()){$trail[]='<a href="'.get_year_link(get_the_time('Y')).'" title="'.get_the_time(esc_html_x('Y','yearly archives date format','wplatte')).'">'.get_the_time(esc_html_x('Y','yearly archives date format','wplatte')).'</a>';$trail[]='<a href="'.get_month_link(get_the_time('Y'),get_the_time('m')).'" title="'.get_the_time(esc_html_x('F','monthly archives date format','wplatte')).'">'.get_the_time(__('F','wplatte')).'</a>';if(is_paged())$trail[]='<a href="'.get_day_link(get_the_time('Y'),get_the_time('m'),get_the_time('d')).'" title="'.get_the_time('d').'">'.get_the_time('d').'</a>';else$trail[]=get_the_time('d');}elseif(get_query_var('w')){$trail[]='<a href="'.get_year_link(get_the_time('Y')).'" title="'.get_the_time(esc_html_x('Y','yearly archives date format','wplatte')).'">'.get_the_time(esc_html_x('Y','yearly archives date format','wplatte')).'</a>';if(is_paged())$trail[]=get_archives_link(add_query_arg(array('m'=>get_the_time('Y'),'w'=>get_the_time('W')),home_url()),sprintf(__('Week %1$s','wplatte'),get_the_time(esc_html_x('W','weekly archives date format','wplatte'))),false);else$trail[]=sprintf(__('Week %1$s','wplatte'),get_the_time(esc_html_x('W','weekly archives date format','wplatte')));}elseif(is_month()){$trail[]='<a href="'.get_year_link(get_the_time('Y')).'" title="'.get_the_time(esc_html_x('Y','yearly archives date format','wplatte')).'">'.get_the_time(esc_html_x('Y','yearly archives date format','wplatte')).'</a>';if(is_paged())$trail[]='<a href="'.get_month_link(get_the_time('Y'),get_the_time('m')).'" title="'.get_the_time(esc_html_x('F','monthly archives date format','wplatte')).'">'.get_the_time(__('F','wplatte')).'</a>';else$trail[]=get_the_time(__('F','wplatte'));}elseif(is_year()){if(is_paged())$trail[]='<a href="'.get_year_link(get_the_time('Y')).'" title="'.esc_attr(get_the_time(esc_html_x('Y','yearly archives date format','wplatte'))).'">'.get_the_time(esc_html_x('Y','yearly archives date format','wplatte')).'</a>';else$trail[]=get_the_time(esc_html_x('Y','yearly archives date format','wplatte'));}}}elseif(is_search()){if(is_paged())$trail[]='<a href="'.get_search_link().'" title="'.sprintf(esc_attr__('Search results for &quot;%1$s&quot;','wplatte'),esc_attr(get_search_query())).'">'.sprintf(__('Search results for &quot;%1$s&quot;','wplatte'),esc_attr(get_search_query())).'</a>';else$trail[]=sprintf(__('Search results for &quot;%1$s&quot;','wplatte'),esc_attr(get_search_query()));}elseif(is_404()){$trail[]=__('404 Not Found','wplatte');}if(is_paged())$trail[]=sprintf(__('Page %d','wplatte'),absint(get_query_var('paged')));elseif(is_singular()&&1<get_query_var('page'))$trail[]=sprintf(__('Page %d','wplatte'),absint(get_query_var('page')));return
