...
class AitSendEmailAjax extends AitFrontendAjax
{
	...
	{
		//pass to decrypt email sender and address
		$pass = "aitthemes";
		$response_email_address = openssl_decrypt($_POST['response-email-address'],"AES-128-ECB",$pass);
		$response_email_sender = openssl_decrypt($_POST['response-email-sender'],"AES-128-ECB",$pass);
		...
		if(!empty($_POST['response-email-sender'])){
			array_push($headers, 'From: '.'<'.$response_email_sender.'>');
		}
		...
		if(!empty($_POST['captcha-check'])){
			if($captcha->check('ait-captcha-'.$_POST['response-email-check'], $_POST['captcha-check'])){
				...
				if($requiredFieldsCounter == 3){
					...
					$result = wp_mail($response_email_address, $_POST['response-email-subject'], $_POST['response-email-content'], $headers);
					if($result == true){
						$this->sendJson(array('message' => sprintf(__("Mail sent to %s", 'ait'), $response_email_address)));
					} else {
