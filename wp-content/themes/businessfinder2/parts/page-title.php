...
{if $wp->is404 or $wp->isSearch or $wp->isWoocommerce()}
	...
	{* TITLE ********** *} {if $wp->isSearch}
								{if isset($_REQUEST['a']) && $_REQUEST['a'] != ""}
									{var $sString = array()}
									{if isset($_REQUEST['s']) && $_REQUEST['s'] != ""}{? array_push($sString, stripcslashes(htmlspecialchars($_REQUEST['s'])) )}{/if}
...
{elseif $wp->isPage or $wp->isSingular(post) or $wp->isSingular(portfolio-item) or $wp->isSingular(event) or $wp->isSingular(job-offer) or $wp->isSingular(item) or $wp->isSingular(event-pro) or $wp->isSingular(ait-special-offer) or $wp->isAttachment}
{loop as $post}
	...
	{* META DATA ****** *} {if $wp->isSingular(event)}			{var $meta = $post->meta(event-data)}
						   ...
						   {elseif $wp->isSingular(ait-special-offer)}	{var $meta = $post->meta(special-offer-data)}
						   {/if}

	{* TITLE ********** *} {var $titleName = $post->title}
	{* IMAGE ********** *} {var $titleImage = $post->imageUrl}
						   {if $wp->isAttachment or $wp->isSingular(portfolio-item) or $wp->isSingular(job-offer) or $wp->isSingular(post) or $wp->isPage or $wp->isSingular(event) or $wp->isSingular(item) or $wp->isSingular(event-pro) or $wp->isSingular(ait-special-offer)} {var $titleImage = ''} {/if}
	...
	{* VALIDITY ******* *} {if $wp->isSingular(job-offer)}
								...
								{/if}
						   {/if}
	{* DATE INTERVAL ** *} {if $wp->isSingular(ait-special-offer)}	{capture $intLabel}{__ 'Duration:'}{/capture}
											{var $intFrom = $meta->dateFrom}
											{var $intTo = $meta->dateTo}
											{if $intTo}{var $dateInterval = 'yes'}{/if}
	{/if}
	...
	{* ITEM ICON ****** *} {if $wp->isSingular(item)}
								{var $terms = get_terms(array('parent' => 0, 'taxonomy' => 'ait-items', 'hide_empty' => false))}
								{var $defaultIcon = $options->theme->items->categoryDefaultIcon}
								{var $termsWithIcons = aitListCategoriesWithIcons($terms, 'ait-items', $defaultIcon, 'icon')}
								{var $categoryData = aitItemCategoriesData($post->id, $defaultIcon, $termsWithIcons)}
